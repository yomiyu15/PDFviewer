...
app.post('/edit-subfolder', (req, res) => {
    ...
    if (!parentFolderName || !currentSubfolderName || !newSubfolderName) {
        return res.status(400).send('Parent folder name, current subfolder name, and new subfolder name are required');
    }
    ...
    if (!fs.existsSync(currentSubfolderPath)) {
        return res.status(404).send('Current subfolder does not exist');
    }
...
// Endpoint to delete a subfolder
app.delete('/delete-subfolder', (req, res) => {
    ...
    const subfolderPath = path.join(__dirname, 'uploads', parentFolderName, subfolderName);
