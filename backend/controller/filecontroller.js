...
const uploadFile = async (req, res) => {
    ...
    const parentDir = path.join(UPLOADS_DIR, sanitizedParentName);
    const subDir = path.join(parentDir, sanitizedSubfolderName);

    try {
        // Check if parent and subfolder exist
        await fs.access(parentDir);
        await fs.access(subDir);
    } catch (err) {
        return res.status(400).send('Parent or subfolder does not exist');
    }
